const isLeapYear = (receivedYear: number): boolean => {
  if (
    receivedYear % 400 === 0 ||
...
const isInvalidMonthDay = (receivedMonth: number, receivedDay: number): string | false => {
  const daysInMonths = {
...
const testDates = (receivedDate: string): string | false => {
  try {
    const year = receivedDate.split(/[/.-]/g)[0];
    if (!year || year.length < 4) return 'Não foi informado um ano corretamente.';
    ...
    if (!day || day.length < 2) return 'Não foi informado um dia corretamente.';
    if (isLeapYear(numberYear) && numberMonth === 2 && numberDay > 29) {
      return 'Para um ano bissexto, o mês de fevereiro tem apenas 29 dias.';
    }
    if (numberMonth === 2 && numberDay > 28 && !(isLeapYear(numberYear))) {
      return 'Para ano não bissexto, o mês de fevereiro tem apenas 28 dias.';
    }
    if (isInvalidMonthDay(numberMonth, numberDay)) return isInvalidMonthDay(numberMonth, numberDay);
